...
export const aiConnectionsApi = {
  ...
  // magmarta fork policy (g): drive the provider CLI from the browser instead
  // of a terminal on the host. See .github/FORK-POLICY.md.
  spawnBrowserLogin: (companyId: string, sessionId: string, input: AiConnectionLoginIntent) =>
    api.post<{ url: string }>(`/companies/${companyId}/ai-connections/local/attempts/${sessionId}/spawn`, input),
  submitBrowserLoginCode: (companyId: string, sessionId: string, input: AiConnectionLoginIntent & { code: string }) =>
    api.post<{ ok: boolean; detail: string }>(`/companies/${companyId}/ai-connections/local/attempts/${sessionId}/code`, input),
